...
const TopCategories = async () => {
    ...
    const categoryData:ICatData[] = await client.fetch(`*[_type == "categories"]{
  _id,
  title,
  price,
  ...
  return (
    <div className="mx-auto max-w-7xl flex flex-col gap-10 mb-40">
        ...
        <div className="flex gap-4 flex-wrap xl:flex-nowrap justify-center lg:justify-normal">
            {
                categoryData.map((data:ICatData, index:number)=>{
                    return(
                        ...
                        <Link href={`/shop/${data.slug}`} className="px-6 py-2 bg-white text-black rounded-md hover:bg-gray-400 transition-colors">
                            Shop Now
                        </Link>
                </div>
            </div>
